...
import { ContactFormData, BusinessCard, Coworker, Contact } from '@/types';
...
const EditContactPage: React.FC = () => {
  ...
  const loadContactData = async (contactId: number) => {
    ...
    try {
      ...
      reset({
        ...
        raw_text: contactData.raw_text || '',
        summary_text: contactData.summary_text || '',
      });

      // 担当者と同席者を設定
      setSelectedPersons(contactData.persons || []);
      setSelectedCompanions(contactData.companions || []);
    } catch (error) {
  ...
  const onSubmit = async (data: FormData, status: number) => {
    ...
    try {
      ...
      await contactAPI.update(parseInt(id as string), contactData);
  ...
  const handleCancel = () => {
    if (window.confirm('編集内容を破棄して下書き一覧に戻りますか？')) {
      router.push('/drafts');
    }
  };
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-primary-700 mb-2">編集</h1>
          <p className="text-gray-600">面談記録を編集します</p>
        </div>
